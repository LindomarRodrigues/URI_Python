...
data_inicial = str(dia_inicio)+' '+str(3)+' '+str(2000)+' '+str(hora_inicio)+' '+str(min_inicio)+' '+str(seg_inicio)
...
data_final = str(dia_final)+' '+str(3)+' '+str(2000)+' '+str(hora_final)+' '+str(min_final)+' '+str(seg_final)
data_final = datetime.strptime(data_final,"%d %m %Y %H %M %S")


data = str(data_final - data_inicial).replace(' days,', '').replace(':',' ').replace('day,','').split()
